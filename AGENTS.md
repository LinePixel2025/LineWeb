...
个人网站/CMS 单体仓库，GitHub Primer 风格的纯 CSS 前端设计系统（`.gh-*` BEM，已全面改版完成）。React 19 SPA 前端，Express 4 REST API 后端，Python 3 WebSocket 文件存储节点。SQLite 本地开发，PostgreSQL 生产环境。功能：文章/评论、动态页面构建器、网盘、计算器、屏幕时间追踪、AI 助手、头像裁剪上传、PWA。
...
├── storage-node/     # Python 3 WebSocket 文件存储客户端（9 个命令）→ D:/LineWebDrive，仅本地
├── scripts/          # 14 个运维脚本（部署、开发启动/停止、webhook、截图、屏幕时间推送）
├── docs/             # API 参考、Drive 部署指南、屏幕时间 API 指南
├── .omo/             # OpenCode 会话续传数据
...
├── CLAUDE.md         # 用户维护的 Claude Code 指令文件（未跟踪，与 AGENTS.md 并存）
...
| 部署流水线 | `.github/workflows/deploy.yml` | push master → SSH 至 123.207.8.77 `/home/Lineweb`；`git fetch` 失败自动重试 ×3 + 强制 HTTP/1.1；`reset --hard` → `build --no-cache --pull`（先构建不中断服务）→ `down` → `up --force-recreate`；`command_timeout: 30m`；`set -e` 任意失败立即退出 |
...
| 认证（JWT + API Key） | `server/src/middleware/auth.ts` | 双认证 + `?token=` 查询参数；**12 个公开路径**在 `index.ts` 白名单；新增公开端点必须同步改白名单 |
| 屏幕时间认证 | `server/src/middleware/screenTimeAuth.ts` | `X-Screen-Time-Token` 头单独认证，用于 `/api/health/push` |
| AI 助手 | `server/src/routes/ai.ts` + `services/aiService.ts` | OpenAI SDK + 兼容 baseUrl；`AiConfig` 单行表存 key（脱敏显示）；**`/api/ai/chat` 公开无认证**（POST），`/chat/public` GET 取站点内容；管理端 `/admin/ai` 配置 |
...
| 设计系统 | `client/src/styles/`（8 个 CSS 文件） | `globals.css` 仅 8 行 import 聚合器；`drive.css` 最大（2402 行）；无 Tailwind、无 CSS-in-JS |
...
git push origin master
   → GitHub Actions SSH 至 123.207.8.77 /home/Lineweb（command_timeout 30m）
      ...
      → docker restart 1Panel-openresty（清 Nginx/OpenResty 静态缓存）
...
- **根目录无 `npm test`** — 仅在 `client/` 中有测试（16 个文件，7 个 `__tests__/` 目录）；server 零测试
...
- **超大文件**：`drive.ts`（894 行）、`drive.css`（2402 行）、`PageEditor.tsx`（733 行）
...
- **未跟踪文件**：`CLAUDE.md`、`client/admin-new.png` 在 git status 中未跟踪
...
- **AI chat 接口公开** — `/api/ai/chat` 无认证（消耗 API key 配额，滥用风险）
...
# 测试（仅 client/）
cd client && npm run test        # vitest run（16 个测试文件）
...
- **Storage node 是外部进程** — Python 进程运行在本地 Windows 机器上，未容器化。通过 WebSocket 以指数退避重连（最长 60s）方式连接到 `/ws/storage`；认证 5s 超时、心跳 30s。
